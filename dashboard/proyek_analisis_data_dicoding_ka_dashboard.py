...
df = pd.read_csv("https://raw.githubusercontent.com/kamilatulgit/bikesharingka-app/refs/heads/main/dashboard/data_.csv")
...
with st.sidebar:
    st.image("https://github.com/kamilatulgit/bikesharingka-app/blob/cf193756d894aaa4a7a405ba132176f0d9f7516a/dashboard/bike-sharing.jpeg")
...
hour_count_df = main_df.groupby("hours")["count_cr"].sum().reset_index()
...
season_df = main_df.groupby("season")["count_cr"].sum().reset_index()
...
with col1:
    st.metric("Total Peminjaman", value=main_df["count_cr"].sum())
with col2:
    st.metric("Jumlah Data", value=len(main_df))